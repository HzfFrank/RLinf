...
class DaggerRolloutWorker(Worker):
    ...
    def predict(self, env_obs, mode="train"):
        ...
        # beta is probability of using learned policy (hf_model)
    ...
    def update_intervene_actions(self, env_output, forward_inputs):
        ...
        if intervene_actions is not None:
            if "action" in forward_inputs:
                ...
                # only does Unnormalize, this should work. If there are other transforms,
                ...
                raise NotImplementedError(
                    f"forward_inputs must contain 'action' for intervene_actions to work. "
        ...
        for epoch_idx in tqdm(
            ...
            for _ in range(n_chunk_steps):
                for stage_id in range(self.num_pipeline_stages):
                    ...
                    # Store current forward_inputs (with action added) for next step's intervene handling
                    last_forward_inputs[stage_id] = current_forward_inputs
            ...
            for stage_id in range(self.num_pipeline_stages):
                ...
                if should_save:
                    ...
                    # 如果 rewards 为 None (例如, 在 reset 步骤), 不添加
                ...
                if should_save and "prev_values" in result:
